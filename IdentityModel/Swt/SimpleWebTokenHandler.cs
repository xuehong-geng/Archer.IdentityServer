...
namespace Thinktecture.IdentityModel.Tokens
{
    ...
    public class SimpleWebTokenHandler : SecurityTokenHandler
    {
        ...
        private static string CreateUnsignedToken(SimpleWebToken swt)
        {
            ...
            sb.AppendFormat("Issuer={0}&", WebUtility.UrlEncode(swt.Issuer));
            sb.AppendFormat("Audience={0}&", WebUtility.UrlEncode(swt.AudienceUri.AbsoluteUri));
        ...
        public override ReadOnlyCollection<ClaimsIdentity> ValidateToken(SecurityToken token)
        {
            ...
            foreach (var claim in swt.Claims)
            {
                ...
                value.Split(',').ToList().ForEach(v => id.AddClaim(new Claim(claim.Type, WebUtility.UrlDecode(v), ClaimValueTypes.String, issuerName)));
            }
        ...
        internal static void CreateClaims(SimpleWebToken swt, StringBuilder sb)
        {
            ...
            foreach (var kv in claims)
            {
                ...
                foreach (var val in values)
                {
                    ...
                    {
                        s = WebUtility.UrlEncode(val);
                        continue;
                    }

                    s += string.Format(",{0}", WebUtility.UrlEncode(val));
                }

                sb.AppendFormat("{0}={1}&", WebUtility.UrlEncode(kv.Key), WebUtility.UrlEncode(s));
            }
        }
        ...
        NameValueCollection ParseToken(string encodedToken)
        {
            ...
            foreach (string nameValue in encodedToken.Split('&'))
            {
                ...
                string key = WebUtility.UrlDecode(keyValueArray[0].Trim());               // Names must be decoded for the claim type case
                string value = WebUtility.UrlDecode(keyValueArray[1].Trim().Trim('"')); // remove any unwanted "
